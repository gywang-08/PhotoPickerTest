...
public class PhotoGridAdapter extends SelectableAdapter<PhotoGridAdapter.PhotoViewHolder> {
    ...
    public PhotoGridAdapter(Context context, RequestManager requestManager, List<PhotoDirectory> photoDirectories, ArrayList<String> orginalPhotosPath,ArrayList<Integer> orginalPhotosId, int colNum) {
        this(context, requestManager, photoDirectories);
        setColumnNumber(context, colNum);
        selectedPhotosPath = new ArrayList<>();
        if (orginalPhotosPath != null)
            selectedPhotosPath.addAll(orginalPhotosPath);
        selectedPhotosId = new ArrayList<>();
        if(orginalPhotosId != null)
            selectedPhotosId.addAll(orginalPhotosId);
    }
            ...
            if (canLoadImage) {
                ...
                if (Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                    Uri uri = MediaStore.Images.Media
                            ...
                            .buildUpon()
                            .appendPath(String.valueOf(photo.getId())).build();
                    ...
                    if (onItemCheckListener != null) {
                        if(Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q){
                            isEnable = onItemCheckListener.onItemCheck(pos, photo,
                                    getSelectedPhotosId().size() + (isSelected(photo) ? -1 : 1));
                        } else {
                            isEnable = onItemCheckListener.onItemCheck(pos, photo,
                                    getSelectedPhotosPath().size() + (isSelected(photo) ? -1 : 1));
                        }
                    }
                    if (isEnable) {
    ...
    public ArrayList<String> getSelectedPhotoPaths() {
        ...
        for (String photo : selectedPhotosPath) {
    ...
    public ArrayList<Integer> getSelectedPhotoId() {
        ArrayList<Integer> selectedPhotoId = new ArrayList<>(getSelectedItemCount());

        for (int photo : selectedPhotosId) {
            selectedPhotoId.add(photo);
        }
